...
fn swap_field_elements(max_field: &mut Vec<Vec<usize>>, rng: &mut impl Rng) -> Vec<Vec<usize>> {
    ...
    // println!("or: {:?}", max_field);
    // println!("sw: {:?}", swapped_field);
    swapped_field
}

fn get_zscore_sorted_idx(tanes: &Vec<Vec<i32>>) -> VecDeque<usize> {
...
fn process_local_input(tanes: &Vec<Vec<i32>>, field: &Vec<Vec<usize>>) -> Vec<Vec<i32>> {
    input_interactive! {
        mut u: [[String; N -1];N],
        mut v: [[String; N];N - 1],
    }
    ...
    for i in 0..N {
        for j in 0..N {
            let mut tane1 = Vec::new();
            let mut tane2 = Vec::new();
            if j + 1 < N {
                for k in 0..M {
                    let ch = u[i][j].chars().nth(k).unwrap();
                    if ch == '0' {
                        tane1.push(tanes[field[i][j]][k]);
                    } else if ch == '1' {
                        tane1.push(tanes[field[i][j + 1]][k]);
                    }
                }
            }
            if i + 1 < N {
                for k in 0..M {
                    let ch = v[i][j].chars().nth(k).unwrap();
                    if ch == '0' {
                        tane2.push(tanes[field[i][j]][k]);
                    } else if ch == '1' {
                        tane2.push(tanes[field[i + 1][j]][k]);
                    }
                }
            }
            if !tane1.is_empty() {
...
fn main() {
    ...
    let mut tanes = Vec::new();
    let mut max_field = Vec::new();
    let mut rng = rand::thread_rng();
    for i in 0..T {
        if i == 0 {
            input_interactive! {
                mut initial_tanes: [[i32; M]; NX],
            }
            tanes = initial_tanes; // 初回のみ初期化
        } else {
            tanes = process_local_input(&tanes, &max_field);
        }
        let start_time = Instant::now();
        let mut max_score = calc_score(&tanes);
        let mut z_sorted_idx = get_zscore_sorted_idx(&tanes);
        let mut swapped_field = Vec::new();
        max_field = create_field(&mut z_sorted_idx);

        while start_time.elapsed().as_secs_f64() < TIME_LIMIT {
            swapped_field = swap_field_elements(&mut max_field, &mut rng);
        ...
        for i in 0..N {
            for j in 0..N {
                write!(file, "{} ", max_field[i][j]).unwrap();
            }
            writeln!(file).unwrap();
        }
    }
